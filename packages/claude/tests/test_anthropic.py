...
async def test_walk_request_string_system(conn):
    ...
    assert _url_masked(result["system"])
...
async def test_walk_request_string_content(conn):
    ...
    result = await walk_request(body, conn, "s1", 8)
    assert _url_masked(result["messages"][0]["content"])
...
async def test_walk_request_block_content_text_and_tool_result(conn):
    ...
    assert _url_masked(blocks[0]["text"])
    assert _url_masked(blocks[1]["content"])
...
def test_walk_response_text_block():
    ...
    assert result["content"][0]["text"] == f"key is {_API_SECRET} ok"
...
async def test_forward_complete_unmasks_text_response(conn):
    response_body = {**_EMPTY_RESPONSE, "content": [{"type": "text", "text": f"key is {_API_TOKEN} ok"}]}
...
async def test_streaming_tool_use_json_split_across_deltas(conn):
    stream = _sse([
        ...
        ("content_block_delta", {"type": "content_block_delta", "index": 0, "delta": {"type": "input_json_delta", "partial_json": '{"command": "echo __AWS'}}),
